...
const CatItDetMain = ({ auctionItem, auctionBidInfo, seller, biddingMember, infoExtension, sellerDetailInfo, auctionImages }) => {
  ...
  const handleBidNow = () => {
    ...
    // 백엔드로 데이터 전송
    axios.post(`${process.env.REACT_APP_BACK_SERVER}:8080/auctionDetail/category-item-detail/${auctionItem.auctionIndex}`, biddingData, {
      headers: {
  ...
  const handleBuyNow = () => {
    ...
    // 백엔드로 데이터 전송
    axios.post(`${process.env.REACT_APP_BACK_SERVER}:8080/auctionDetail/category-item-detail/${auctionItem.auctionIndex}`, buyingData, {
      headers: {
        'Content-Type': 'application/json', // 요청의 콘텐츠 타입을 JSON으로 지정
      }
    })
